...
iqs = read.csv(file="/word_proc_docs/q370/IQsPurdueIU.txt",header=T,sep=" ")
...
recalled<-read.table("/word_proc_docs/q370/contextreinstatement.txt", header = TRUE,stringsAsFactors=TRUE) #read in data from context-reinstatement experiment, set stringsAsFactors so that text descriptions are automatically encoded as factor levels
